...
describe("readAnswer", () => {
  ...
  it("never reports a parsed line as a timeout, whatever it says", () => {
    const lines = ['{"ok":false,"error":"nope"}', "nonsense", "[1,2]", '{"ok":true,"result":7}'];

    expect(lines.map((line) => readAnswer(line).timedOut)).toEqual([false, false, false, false]);
  });
