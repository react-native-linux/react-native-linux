...
public:
    ...
    /**
     * Writes one already-newline-terminated response line, whole. A short write is retried and a full send
     * buffer is waited on, because a line delivered without its terminator is indistinguishable to the driver
     * from no line at all. Only a real error, or a driver that stops reading for `kWritableTimeoutMilliseconds`,
     * counts as a lost driver.
     */
...
private:
    ...
    /** Closes and removes a socket that was bound but could not be made usable, and throws naming the syscall. */
    [[noreturn]] void abandonBoundSocket(const char* syscallName);

    /** Whether the client drained its receive buffer before the send timeout. */
    bool waitUntilWritable() const;

    std::string socketPath_;
    int listenDescriptor_;
    int clientDescriptor_{-1};
    AutomationLineBuffer received_;
};
