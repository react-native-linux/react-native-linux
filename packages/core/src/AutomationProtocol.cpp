...
bool AutomationLineBuffer::append(std::string_view bytes) {
    if (pending_.size() + bytes.size() > kMaxRequestBytes) {
        pending_.clear();
...
std::optional<std::string> AutomationLineBuffer::takeLine() {
    const size_t terminator = pending_.find(kLineTerminator);

    if (terminator == std::string::npos) {
        return std::nullopt;
    }

    std::string line = pending_.substr(0, terminator);
...
void AutomationLineBuffer::clear() { pending_.clear(); }
