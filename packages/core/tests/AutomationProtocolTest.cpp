...
TEST(AutomationLineBuffer, TakesOneWholeLineAtATime) {
    AutomationLineBuffer buffer;

    EXPECT_FALSE(buffer.takeLine().has_value());
    EXPECT_TRUE(buffer.append("{\"command\":\"ListErrors\"}\n{\"command\":\"MarkTestPassed\"}\n"));
    EXPECT_EQ(buffer.takeLine().value(), "{\"command\":\"ListErrors\"}");
    EXPECT_EQ(buffer.takeLine().value(), "{\"command\":\"MarkTestPassed\"}");
    EXPECT_FALSE(buffer.takeLine().has_value());
}

TEST(AutomationLineBuffer, WaitsForTheTerminatorOfALineThatArrivedInPieces) {
    AutomationLineBuffer buffer;

    EXPECT_TRUE(buffer.append("{\"command\":"));
    EXPECT_FALSE(buffer.takeLine().has_value());
    EXPECT_TRUE(buffer.append("\"ListErrors\"}\n"));
    EXPECT_EQ(buffer.takeLine().value(), "{\"command\":\"ListErrors\"}");
}

TEST(AutomationLineBuffer, RefusesBytesThatWouldTakeARequestPastTheProtocolLimit) {
    AutomationLineBuffer buffer;

    EXPECT_TRUE(buffer.append(std::string(kMaxRequestBytes, 'x')));
    EXPECT_FALSE(buffer.append("x"));

    // The refused bytes are dropped whole rather than truncated, so nothing half-read is left for the next
    // client the socket accepts.
    EXPECT_FALSE(buffer.takeLine().has_value());
    EXPECT_TRUE(buffer.append("{}\n"));
    EXPECT_EQ(buffer.takeLine().value(), "{}");
}

TEST(AutomationLineBuffer, ForgetsAPartialLineWhenItIsCleared) {
    AutomationLineBuffer buffer;
